...
namespace Inworld
{
    ...
    public class AudioCapture : MonoBehaviour
    {
        ...
        public Action<float> OnAmplitude;
...
#region Public Functions
        ...
        public void ChangeInputDevice(string deviceName)
        {
            if (deviceName == m_DeviceName)
                return;
            if (IsRecording) StopMicrophone(m_DeviceName);
...
#region MonoBehaviour Functions
        ...
        protected virtual IEnumerator AudioCoroutine()
        {
            while (true)
            {
                if (!IsRecording) StartMicrophone(m_DeviceName);
                if (m_BackgroundNoise <= 0f) yield return Calibrate();
...
#region Protected Functions
        ...
        protected virtual IEnumerator Collect()
        {
            ...
            int nSize = GetAudioData();
            if (nSize <= 0) {
                //in built VisionOS applications, other windowed MR apps can 'steal' microphone access
                //we detect this here and steal it back!
                #if UNITY_VISIONOS && !UNITY_EDITOR
                if (m_nPosition == m_LastPosition && nSize == -1) StartMicrophone(m_DeviceName);
                #endif
                yield break;
            }

            float amplitude = CalculateAmplitude();
            IsPlayerSpeaking = amplitude > m_BackgroundNoise * m_PlayerVolumeThreshold;
        ...
        protected float CalculateAmplitude()
        {
            ...
            float amplitude = nCount == 0 ? 0 : fAvg / nCount;
        ...
        protected void StopMicrophone(string deviceName)
        {
            ...
            if(m_Recording) Destroy(m_Recording);
#endif
        }
#endregion
    }
